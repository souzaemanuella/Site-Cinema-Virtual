...
function mostrarFilmes(genero) {
    ...
    filmes[genero].forEach((f, index) => {
        // Cria o card dinamicamente
        const card = document.createElement("div");
        card.classList.add("card_filmes");
        card.dataset.filmeId = index; // ou algum ID único se você tiver
        ...
        btnComprar.addEventListener("click", () => {
            window.location.href = `compra.html?filmeId=${card.dataset.filmeId}`;
        });

        // Adiciona evento de clique à imagem (se quiser abrir a bilheteria)
